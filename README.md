...
- **File links in case studies point at real files** — file and folder references
  in a case study (and in GitItGPT answers) link to the actual file on GitHub,
  verified against the repository's file tree so they never land on a 404. Older
  repositories ingested before this feature get their tree captured the next time
  you use **Refresh all**.
